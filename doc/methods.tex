...
using much memory, and on \index{Lagranges' Theorem}Lagranges' Theorem,
...
\index{Elliptic Curves Method (ECM)}
...
<Limit1> is the initial \index{first stage limit}first stage limit,
<Limit2> is the initial \index{second stage limit}second stage limit and
...
in the \index{elliptic curve groups}elliptic curve groups 
\index{E(a,b)/n}$E(a,b)/n$ is fast enough
...
which states that for each\index{elliptic curve point}
...
However, the calculations are done in\index{projective coordinates}
...
calculations than the \index{Weierstrass model}Weierstrass model 
...
\index{Continued Fraction Algorithm (CFRAC)}
...
in this case). If $x_i/y_i$ is a\index{continued fraction approximation}
...
completely over a chosen \index{factor base}factor base 
...
\lq large factor' is found.\index{large factors (factor base)}
Then, \index{Gaussian Elimination} Gaussian Elimination over $GF(2)$ 
...
\index{Multiple Polynomial Quadratic Sieve (MPQS)}
...
an appropriately chosen \index{sieving interval}sieving interval 
